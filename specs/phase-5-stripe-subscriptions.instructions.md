...
- Memories — `STRIPE_ENABLED=false`: pass; `STRIPE_ENABLED=true` + subscribed: pass; `STRIPE_ENABLED=true` + no subscription: ❌ Paywall (memories remain premium-only, no freemium access)
...
Memories endpoints still use plain `require_subscription` — freemium users cannot access saved LLM memories.
...
- 12 — Task: Apply `require_subscription_or_freemium` to chat/listening/reading/conversation/lessons; memories keep `require_subscription`; File(s): `routers/chat.py`, `conversation.py`, `listening.py`, `reading.py`, `lessons.py`; Status: ✅
