...
966 backend tests, >= 70% backend coverage required. Current backend coverage is 84.79% last measured. SQLite in-memory, no Docker needed.
...
- Step 1: `pytest` runs 966 backend tests with SQLite in-memory.
- Step 2: `eslint` verifies no remaining JS/TS lint errors.
- Step 2: `tsc --noEmit` runs TypeScript type checking.
- Step 2: `vitest` runs 440 frontend tests; frontend coverage is not configured.
